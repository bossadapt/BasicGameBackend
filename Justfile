...
build-and-deploy:
  ...
  ssh debian@ameo.dev -t "cat /tmp/parkour-backend.tar.bz2 | bunzip2 | docker load && docker kill parkour-backend && docker container rm parkour-backend && docker run -d --name parkour-backend -p 5800:8080 --restart always -v /opt/parkour/db/players.db:/app/players.db parkour-backend:latest && rm /tmp/parkour-backend.tar.bz2" && rm /tmp/parkour-backend.tar.bz2
